...
test("workerd makes guild draw public while status remains ephemeral", async () => {
  ...
  try {
    const draw = { id: "111111111111111111", application_id: "123456789012345678", type: 2, token: "test-token", guild_id: "444444444444444444", channel_id: "555555555555555555", member: { user: { id: "987654321098765432" } }, data: { name: "draw", options: [{ name: "prompt", value: "workerd regression" }] } };
    ...
    assert.equal(requests[0].payload.mode, "natural");
