...
This Worker accepts only signed Discord interaction webhooks from an explicit configured Discord user allowlist. It has no database, queue, GPU access, Core access, or generation logic. It forwards accepted commands to the authenticated local bridge reachable through a HTTPS Cloudflare Tunnel.
