...
갤러리 item은 `id,task_id,group_id,work_id,character_id,outfit_id,created_at,generation_image_id,sha256,bytes,media_type,output_path,single_outcome,single_validation_run_id,single_validation_shared_from_image_id,group_status,group_validation_run_id,group_reference_revision,content_url`이다. `content_url`은 `/v1/images/{id}/content`의 인증된 상대 API 경로다. 목록 자체는 이미지 bytes, 전체 Prompt/snapshot, Provider secret을 반환하지 않는다. `output_path`는 Generation이 보고한 ComfyUI 출력 파일 위치이며 과거 결과는 `null`이다(ADR-0026). `media_type` 필터는 image/png 또는 image/webp다.

`single_outcome`은 최신 단일 검증 요청 기준 `unvalidated/pending/passed/failed/error/cancelled`이며, 과거 합격 뒤 새 queued 요청이 있으면 pending이다. `group_status`는 `reference/matched/mismatch/insufficient/reference_conflict/error/unvalidated/stale/not_eligible`로 별도 표시한다. reference는 선택된 기준 역할이지 합격 판정이 아니다. stale은 현재 기준으로 유효한 완료 판정이 없는 과거 기준 결과이고, not_eligible은 현재 묶음 비교 대상에서 제외된 상태다. 현재 기준 결과·대상 집합은 기존 Core group consistency의 판정을 재사용한다. 단일/묶음 상태를 하나의 ambiguous state나 종합 합격으로 합치지 않는다. `single_validation_run_id`와 `group_validation_run_id`는 해당 요약의 근거 실행 ID이며 없으면 null이다. 같은 생성 시도의 대표 이미지 결과를 공유하는 이미지는 `single_validation_run_id`가 대표 이미지의 run이고 `single_validation_shared_from_image_id`에 대표 이미지 ID가 있으며, 그 외에는 null이다(ADR-0025 E). `group_reference_revision`은 현재 기준 revision이므로 stale 결과의 과거 revision으로 해석하지 않는다. 전체 이력은 기존 이미지 검증/그룹 상세 API에서 조회한다.
...
Task/제작 계획의 preview 입력은 선택 `consistency: {method, params?}`를 받는다. 생략하면 대상 의상의 참조 세트가 `valid`일 때 기본 방식 `anima-incontext-character`와 기본 params가 자동 적용되고, `valid`가 아니면 아무 것도 붙지 않는다. `consistency: null`(끔)은 단순 미리보기(`POST /v1/prompts/preview`)에서만 허용하며, 참조 세트가 `valid`인 상태의 실제 Task 생성(`POST /v1/tasks`)·제작 계획 생성(`POST /v1/production-plans`)에서 `consistency: null`을 보내면 400 `CORE_INVALID_INPUT`이다(참조 세트가 있는 한 실제 생성은 항상 일관성 방식을 쓴다). 참조 샘플 접수는 자기 참조를 막기 위해 내부적으로 항상 `consistency: null`로 preview하며 이 제한과 무관하다(예외는 참조 샘플 Task와 과거 snapshot 재생성뿐이다). 현재 방식 params는 `strength`(기본 1.0, 0.5~1.5), `end_percent`(기본 0.5, 0.3~1.0, 0.5 미만이면 응답에는 없지만 서버가 검증만 하고 경고 표시는 Frontend 몫), `suppress_reference_background`(기본 true)다. 방식·범위는 `GET /v1/generation/resources`의 `consistency_methods`(Generation 등록)와 값이 같다(현재 Core는 preview가 동기 함수라 Generation을 매 호출 원격 조회하지 않고 같은 값의 로컬 정의로 검증하며, Generation이 보고하는 `available`/`unavailable_reason`(필요 Node·LoRA 미등록 등)은 아직 확인하지 않는다 — 미등록 상태에서도 Core는 접수를 허용하고 최종 거절은 Generation의 `GEN_NODE_UNAVAILABLE`에 의존한다).
...
Validation 평가 버전은 5다. Core는 실제 `generation_settings.seed/steps/cfg`를 전달하며 provider는 `regeneration_changes`에 `{field,value,reason,evidence_ids}`를 제안한다. 실패 evidence ID에 연결된 seed/steps/cfg의 유효한 변경만 허용한다. ADR-0025 F(2026-09-23)부터 불합격인데 유효한 제안이 없으면(제안 없음·Validation이 버린 제안·Core 재검사 불통과) 상한 안에서 이전 설정 그대로 Seed만 `0..2^53-1` 무작위 새 값으로 바꿔 자동 재생성한다. 자동 시도 Task `regeneration.change`와 cycle `last_change`(추가로 `parent_task_id`)는 `{kind:"seed_only", reason:"proposal_unavailable"|"proposal_dropped"|"proposal_invalid", previous_seed, seed}` 또는 `{kind:"proposal", fields:[...]}`다. 새 cycle은 더 이상 `proposal_unavailable`로 끝나지 않으며 과거 기록의 상태 값으로만 남는다. 과거 형식별 2회 검증 Task의 제안 충돌은 `CORE_REGENERATION_CONFLICT`다.

ADR-0025 E(2026-09-23)부터 자동 단일 검증은 생성 시도(Task)당 대표 이미지 1장(첫 PNG, 없으면 첫 출력)만 요청한다. 검증 run에는 `shared_image_ids`(결과를 공유하는 같은 시도의 다른 형식 이미지 ID)가 붙고, 해당 이미지의 `validation` 요약은 같은 run `id`와 `shared_from_image_id`(대표 이미지 ID)를 가진다. Task `validation`은 `representative_image_id`와 image 항목별 `shared_from_image_id`를 포함한다. 공유 이미지의 `GET /v1/images/{id}/validations`는 자체 실행이 없으므로 빈 목록이며, 수동 `POST /v1/images/{id}/validations`는 그 이미지에만 적용되고 이후 공유 결과로 덮이지 않는다. 과거에 형식별로 따로 검증한 Task는 기존 run을 그대로 사용한다. 자동 프롬프트/모델/LoRA 변경 및 묶음 검증 기반 재생성은 이번 구현 범위에 포함하지 않는다.
