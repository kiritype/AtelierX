...
## 2026-09-24 참조 세트 강제와 일관성 — ADR-0027

`POST /v1/production-plans`는 대상 그룹의 의상에 확정된 `valid` 참조 세트가 있어야 한다. 없거나 `needs_review`면 계획을 저장하지 않고 409 `CORE_REFERENCE_SET_REQUIRED`를 반환한다(이 묶음 대신 [참조 세트와 일관성](rest-api.md#참조-세트와-일관성---adr-0027) 절의 참조 샘플·확정 API로 먼저 세트를 만든다). 요청 본문은 선택 `consistency`(생략 시 참조가 `valid`이면 기본 `anima-incontext-character` 자동 적용, `null`이면 끔)와 `accept_reference_settings_mismatch`를 받는다. 계획의 모든 항목은 같은 생성 설정·공통 조각을 쓰므로 설정 불일치 검사는 계획당 한 번만 수행하고 모든 항목에 같은 결과를 적용한다. 검사·계획 hash에는 `consistency`가 포함된다. 참조 샘플 Task와 과거 snapshot 재생성은 이 강제·일관성 합성에서 제외된다(REST API 문서 참조).
