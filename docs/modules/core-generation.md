...
### 참조 세트와 일관성 — ADR-0027 (2026-09-24)
- [구현] Core `ReferenceSets`(`core/reference_sets.py`)가 의상 단위 참조 세트를 SQLite에 revision 이력으로 보관한다(삭제 없음). 상태는 `none|valid|needs_review`이며 현재 캐릭터·의상 revision과 세트 기록이 다르면 `needs_review`다. 확정·재확인은 각각 새 revision을 추가한다
- [구현] 참조 샘플(`POST /v1/outfits/{id}/reference-samples`)은 전역 설정 `reference_templates`의 고정 구도로 같은 Seed의 전신·얼굴 Task 쌍을 만든다. Task snapshot `purpose:"reference_sample"`은 P5 강제·자동 검증 요청·`consistency` 합성에서 제외하는 표시다
- [구현] `POST /v1/tasks`(참조 샘플 제외)와 `POST /v1/production-plans`는 접수 전 `Core.require_reference_set`으로 대상 의상 참조 세트가 `valid`인지 확인하고, 아니면 409 `CORE_REFERENCE_SET_REQUIRED`로 거절한다. 재생성(`/regenerations`)은 원래 snapshot을 그대로 쓰므로 이 검사를 거치지 않는다 (코드: core/__init__.py `submit`, core/production_plans.py `create`)
- [구현] `Core.preview`가 `consistency`를 합성한다. 생략 시 참조가 `valid`면 기본 `anima-incontext-character`(strength 1.0, end_percent 0.5, suppress_reference_background true)를 붙이고, `valid`가 아니면 아무 것도 붙이지 않는다. `consistency:null`은 preview 계층에서는 항상 허용하지만(참조 샘플 접수가 내부적으로 사용), 참조가 `valid`인 상태의 실제 `Core.submit`/`ProductionPlans.create`는 이를 400 `CORE_INVALID_INPUT`으로 거절한다 — 참조 세트가 있는 한 실제 생성은 일관성을 끌 수 없다(예외: 참조 샘플 Task, 과거 snapshot 재생성). Task snapshot에는 `{method,params,reference_set,references:[{role,core_image_id,generation_image_id,sha256}]}`을, `generation_inputs.consistency`(Generation 전송분)에는 `{method,params,references:[{role,image_id,sha256}]}`을 둔다(Generation 계약은 PR #29 `src/atelierx/generation/consistency.py`). `suppress_reference_background`가 켜지면 최종 Negative 마지막에 `white background, simple background`를 더하고 `negative_sources.consistency`에 기록한다(ADR-0023/0027 P4, global과 같이 검사 제외)
- [제한] 방식·params 유효 범위는 Generation의 `/v1/resources` `consistency_methods`와 동일한 값의 Core 로컬 정적 정의(`core/__init__.py` `CONSISTENCY_METHODS`)로 검증한다. `Core.preview`가 동기 함수라 이번 범위에서는 매 preview마다 Generation을 원격 조회하지 않는다(값은 두 서비스가 지금은 실제로 일치하지만, 두 곳에 나뉘어 있어 향후 Generation 쪽 방식이 추가되면 함께 갱신해야 한다). Core는 Generation이 보고하는 `available`/`unavailable_reason`(필요 ComfyUI Node·LoRA 미등록)을 접수 시점에 확인하지 않는다 — 방식이 형식·범위상 유효하면 계획/Task를 접수하고, 실제 미등록은 Generation 실행 단계의 `GEN_NODE_UNAVAILABLE`(503)로만 드러난다. Frontend가 등록 상태를 미리 보여 주려면 `GET /v1/generation/resources`를 직접 조회해야 한다
- [구현] 설정 불일치 경고(P1/#7): 참조 세트 `settings_summary`(diffusion_model, text_encoder, common_fragments id+revision, positive_quality, loras)와 실제 계획/Task 설정이 다르면 409 `CORE_REFERENCE_SETTINGS_MISMATCH`(`{diff}`)이고, `accept_reference_settings_mismatch:true`면 진행하며 snapshot에 `reference_settings_mismatch_accepted`를 기록한다. 차단이 아니라 확인 절차다
- [구현] `Validation` 서비스는 `negative_sources`에 선택 `consistency` 키를 허용하고 global→character→fragment→consistency 순 재현을 검사한다(코드: `validation.py` `request_body`)
- [제한] Frontend UI는 이번 범위에 없다(참조 세트 화면, 일관성 방식 폼, 불일치 확인 UI는 후속). CLI 연동도 아직 없다
