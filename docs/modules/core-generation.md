...
- [구현] `Core.preview`가 `consistency`를 합성한다. 생략 시 참조가 `valid`면 기본 `anima-incontext-character`(strength 1.0, end_percent 0.5, suppress_reference_background true)를 붙이고, `consistency:null`이면(참조 샘플 자체 포함) 아무 것도 붙이지 않는다. Task snapshot에는 `{method,params,reference_set,references:[{role,core_image_id,generation_image_id,sha256}]}`을, `generation_inputs.consistency`(Generation 전송분)에는 `{method,params,references:[{role,image_id,sha256}]}`을 둔다(Generation 계약은 PR #29 `src/atelierx/generation/consistency.py`). `suppress_reference_background`가 켜지면 최종 Negative 마지막에 `white background, simple background`를 더하고 `negative_sources.consistency`에 기록한다(ADR-0023/0027 P4, global과 같이 검사 제외)
- [제한] 방식·params 유효 범위는 Generation의 `/v1/resources` `consistency_methods`와 동일한 값의 Core 로컬 정적 정의로 검증한다. `Core.preview`가 동기 함수라 이번 범위에서는 매 preview마다 Generation을 원격 조회하지 않는다(값은 두 서비스가 지금은 실제로 일치하지만, 두 곳에 나뉘어 있어 향후 Generation 쪽 방식이 추가되면 함께 갱신해야 한다)
