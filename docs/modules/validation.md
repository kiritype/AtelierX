...
- [구현] Negative 검사 대상은 `negative_sources.character`만이며(2026-09-23 선택 `fragment` 출처는 생성 전용으로 `negative_prompt` 재현 확인에만 쓰고 검사 항목을 만들지 않음) `profile.positive_prompt/negative_prompt=false`면 해당 종류 항목을 만들지 않음.
