...
- 상태: Accepted
- 작성일 / 확정일: 2026-09-11
- 추가 확정일: 2026-09-13 — 기본 생성 해상도 1024×1024, 4배 Upscale 모델 사용 시 기본 최종 배율 1.5배(1536×1536)
- Supersedes: 없음
- Superseded by: 없음
...
| N-02 | 생성 입력은 모델, 다중 LoRA와 개별 가중치, Positive / Negative Prompt, Resolution, CFG, Steps, Sampler, Scheduler, Seed 등의 파라미터이며 출력은 이미지다. 기본 생성 해상도는 **1024×1024**이며 사용자가 명시한 해상도는 이 기본값보다 우선한다. 파라미터 설정을 저장한다. 저장 형식은 미정이며 SQLite 사용도 허용된다. |
...
| N-13 | Upscale은 입력 이미지 외에 모델과 배율만 사용자 설정으로 제공한다. 모델의 고유 배율과 별도로 입력 이미지 대비 최종 배율을 지정한다. 기본값은 UltraSharp 4x 등 **4배 모델**과 최종 배율 **1.5배**이며, 기본 1024×1024 입력은 최종 **1536×1536**이 된다. 모델의 4배 고유 배율은 최종 출력 배율이 아니며, 사용자가 명시한 모델 또는 최종 배율은 이 기본값보다 우선한다. | 고유 배율과 요청 배율을 맞추는 내부 처리·리샘플링·크기 반올림은 미정이다. 타일·Overlap·목표 크기·Lossless 옵션은 현재 요구에서 제외한다. WebP 품질은 N-12로 이동한다. |
...
2026-09-13 사용자 확정으로 N-02와 N-13의 기본값을 추가했다. 기본 생성은 1024×1024이고 4배 모델의 기본 최종 배율은 1.5배여서 최종 1536×1536이다. 이는 모델의 고유 4배와 최종 배율을 구분하는 정책이며, 사용자 명시 해상도·모델·최종 배율을 덮어쓰지 않는다. 모델 설치·가용성이나 내부 배율 처리 방식은 이 결정으로 확정하지 않는다.
