...
상단 제작·갤러리·작업 현황·설정과 기본 배치는 사용자 승인으로 [화면 구성 문서](../../modules/frontend.md)에 보존했다. 메뉴 구성을 다시 미정 질문으로 올리지 않는다. 제작 상세와 기존 API 대조를 진행하며 일반 조회 API·UI 상세를 별도 ADR 승인 질문으로 쪼개지 않는다. Framework·브라우저 재시작 후 초안 저장·삭제/Import/Export 정책 등은 기존 경계에 맞춰 별도로 구분한다.
