...
2026-09-22 공통 적용 프롬프트: Core가 공통 조각 참조·본문·revision을 고정하고 이미지별 조각과 합성한다. 공통 조각은 본문만 추가하며 이미지 수와 의상 포함 규칙은 변경하지 않는다. 작품·캐릭터의 기본 공통 적용 프롬프트 지정은 후속 roadmap이다. 개인용 연결 입력은 F/E 설정에서 일원화하고 일괄 실행은 로그를 확인할 수 있는 Windows 전경 실행을 우선한다.
