...
2026-09-22 공통 적용 프롬프트 확정: 조각을 공통 적용/이미지별 생성으로 명시적으로 구분한다. 공통 적용은 본문만 이번 계획의 모든 이미지에 합성하고 장수를 늘리지 않는다. 의상 포함 옵션은 이미지별 조각에만 적용한다. 이미지 수는 대상 의상 × 이미지별 조각이며 공통만 선택해서는 계획을 만들지 않는다. 기존 조각은 이미지별 조각으로 유지한다. 작품·캐릭터 기본 공통 적용 프롬프트 설정은 roadmap 후속 범위다.

2026-09-22 생성 설정 후속 확정: Seed -1은 Core가 이미지마다 실제 난수를 고정·저장하며 동일 접수 재요청은 같은 값을 유지한다. 생성 화면은 계층별 트리, 작은 체크박스, 업스케일 모델 목록 및 예상 최종 해상도를 제공한다. 대량 제작에서도 생성만/단일 검사/단일+묶음 검사를 선택한다. 검사 생략 결과는 미검사로 조회·보관하고 이후 수동 검사를 허용한다. 기존 제작 계획의 검사 필수 계약은 이 선택 기능으로 확장하며, 묶음 검사는 단일 검사를 전제로 한다.

2026-09-22 최신 사용자 확정: 외형은 캐릭터에 저장하고 의상은 상의·하의·액세서리를 저장한다. 조각은 상의·하의·액세서리 포함 여부(액세서리 기본 포함)를 지정하며 외형은 항상 포함한다. 아래 과거 의상 내부 외형 표현보다 이 정정이 우선한다. 캐릭터 관리/이미지 생성 및 검토/갤러리를 분리한다. 상세 실행 범위는 [UI 개편](../modules/frontend.md)을 따른다.
...
2026-09-21 사용자 승인: 개인용 Frontend는 고정 주소의 Cloudflare Access 로그인 후 Core 토큰을 반복 입력하지 않고 연결한다. 연결 토큰은 서버의 비공개 설정에 저장하고 설정 화면에서 상태 확인·검증 후 교체한다. 토큰 원문을 응답·정적 파일·브라우저 저장소·URL에 넣지 않으며, 서비스 전체 토큰 회전과 구분한다. Google OAuth 전환은 후속으로 미룬다. [적용 및 검증 상태](../development/operations.md).

2026-09-21 Discord 서버 추가: 기존 테스트 서버와 사용자가 추가 지정한 서버를 허용한다. 두 서버의 모든 채널에서 멤버가 사용할 수 있으며 다른 서버와 DM은 차단한다. [운영 반영 상태](../development/discord.md).
...
사용자 승인한 상단 구성은 제작 / 갤러리 / 작업 현황 / 설정이다. 제작에는 작품 > 캐릭터 > 의상 트리, 프롬프트 편집과 생성·후처리 설정을 배치한다. 갤러리에서 이미지·그룹 검토 및 기준 관리를, 작업 현황에서 실행·오류·이력을, 설정에서 전역·Preset·검사·연결 설정을 제공한다. 선택·필터 유지, 미저장 편집 이동 안내와 판정 상태 구분을 포함한다. [상세 구성](../modules/frontend.md)과 [제작 상세 초안](../modules/frontend.md)을 참조한다. 기존 F-01~F-15를 조직한 것이며 새로운 모델 다운로드·삭제 계약을 승인한 것은 아니다.
