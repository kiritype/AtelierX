...
Core SQLite에 단일 profile·묶음 profile·Provider의 생성, 조건부 수정, 복제, 보관, 조회, revision 이력을 구현했다. 단일/묶음 profile ID는 공통 namespace에서 유일하다. 보관한 설정은 새 작업에서 선택할 수 없고 기존 Task는 접수 당시 snapshot을 유지한다. 후속 구현으로 신체 이상 검사는 hands/face/limbs를 선택해 활성화할 수 있다(기본 Off). metadata 검사는 여전히 활성화할 수 없다. [신체 검사 검증](body-structure-validation.md)의 구현·실제 평가 범위를 구분한다.
