...
param(
    ...
    [ValidateNotNullOrEmpty()]
    [string]$LinkName = "atelierx_upscale"
)
...
if (Test-Path -LiteralPath $targetPath) {
    ...
    if ($resolvedTarget -ieq $sourcePath) {
        Write-Output "AtelierX Upscale junction already installed: $targetPath -> $sourcePath"
        return
    }
...
if ($PSCmdlet.ShouldProcess($targetPath, "Create directory junction to $sourcePath")) {
    ...
    $targetItem = Get-Item -LiteralPath $targetPath -Force
    if ((Get-JunctionTargetPath -Item $targetItem) -ine $sourcePath) {
        throw "Created junction does not resolve to the package source: $targetPath"
    }
    Write-Output "Installed AtelierX Upscale junction: $targetPath -> $sourcePath"
}
