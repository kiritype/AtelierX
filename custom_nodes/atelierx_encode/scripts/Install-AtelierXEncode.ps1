...
param(
    ...
    [ValidateNotNullOrEmpty()]
    [string]$LinkName = "atelierx_encode"
)
...
if (Test-Path -LiteralPath $targetPath) {
    ...
    if (-not $isJunction) { throw "Refusing to replace existing non-junction target: $targetPath" }
    $resolvedTarget = Get-JunctionTargetPath -Item $targetItem
    if ($resolvedTarget -ieq $sourcePath) {
        Write-Output "AtelierX Encode junction already installed: $targetPath -> $sourcePath"
        return
    }
...
if ($PSCmdlet.ShouldProcess($targetPath, "Create directory junction to $sourcePath")) {
    ...
    if ((Get-JunctionTargetPath -Item (Get-Item -LiteralPath $targetPath -Force)) -ine $sourcePath) {
        throw "Created junction does not resolve to the package source: $targetPath"
    }
    Write-Output "Installed AtelierX Encode junction: $targetPath -> $sourcePath"
}
