...
async function navigate(page,id){
 ...
 try {const module=await import(`./${page}.js`);if(ticket!==serial)return;host.textContent='';const dispose=await module.mount(host,{api,state:states[page],navigate,notify});if(ticket!==serial){if(typeof dispose==='function')dispose();return;}cleanup=typeof dispose==='function'?dispose:null;}
...
navigate(location.hash.slice(1)||'production');
