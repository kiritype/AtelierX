...
async function navigate(page,id,fromHistory=false,force=false){
 ...
 catch(e){if(ticket===serial){const retry=document.createElement('button');retry.type='button';retry.className='button';retry.textContent='화면 새로고침';retry.onclick=()=>window.location.reload();host.replaceChildren(document.createElement('h2'),document.createElement('p'),retry);const detail=String(e&&e.message||e).slice(0,300);host.children[0].textContent='화면 파일을 불러오지 못했습니다.';host.children[1].textContent=`Cloudflare Access 로그인과 연결 상태를 확인한 뒤, 준비되면 화면을 새로고침하세요. (${page}.js: ${detail})`;notify(`화면 파일을 불러오지 못했습니다(${page}.js): ${detail}`,true);}lifecycles.delete(lifecycle);return;}
