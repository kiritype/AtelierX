...
async function navigate(page,id,fromHistory=false,force=false){
 ...
 try {const module=await import(`./${page}.js`);if(ticket!==serial)return;host.textContent='';const dispose=await module.mount(host,{api,state:states[page],navigate,notify,isActive:()=>ticket===serial&&!lifecycle.controller.signal.aborted,signal:lifecycle.controller.signal,onDetailChange:(id)=>{if(ticket!==serial||lifecycle.controller.signal.aborted)return;lifecycle.detailRoute=routeKey(current,id);activeRoute=lifecycle.detailRoute;history.pushState(null,'',`#${current}${id?'/'+encodeURIComponent(id):''}`);},onConnectionSaved:async()=>{try{await api.get('/health');document.querySelector('#connection-status').textContent='Core 연결됨';notify('저장한 Core 연결을 확인했습니다.');const initial=route();await navigate(current,initial.id,true,true);}catch(error){notify(`${error.code||'CONNECTION_ERROR'}: ${error.message}`,true);}}});lifecycle.cleanup=typeof dispose==='function'?dispose:null;if(ticket!==serial){endLifecycle(lifecycle);return;}activeLifecycle=lifecycle;activeRoute=lifecycle.detailRoute||routeKey(page,id);window.scrollTo({top:states[page].scrollY||0,behavior:'instant'});}
...
document.querySelector('#connection-form').addEventListener('submit',async e=>{e.preventDefault();const button=e.submitter;button.disabled=true;try{const next=new ApiClient({token:document.querySelector('#core-token').value});await next.get('/health');api=next;document.querySelector('#core-token').value='';document.querySelector('#connection-panel').hidden=true;document.querySelector('#connection-toggle').setAttribute('aria-expanded','false');document.querySelector('#connection-status').textContent='Core 연결됨';notify('스튜디오에 연결했습니다.');const initial=route();await navigate(current,initial.id,true,true);}catch(error){notify(`${error.code||'CONNECTION_ERROR'}: ${error.message}`,true);}finally{button.disabled=false;}});
